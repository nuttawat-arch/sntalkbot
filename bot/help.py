...
class HelpCommands:
    ...
    def __init__(self, config_handler):
        ...
        self.commands = {
            ...
            '/voice voiceName': self._('Set the voice name for text to speech.'),
            ...
            '/ttsmode microsoft|google': self._('Switch TTS mode between Microsoft and Google Cloud TTS.'),
            '/get_voices langcode': self._('Get all voices for a specified language (Microsoft) or list Google voices when in Google mode.'),
            ...
            '/pttsmode microsoft|google': self._('Admins only: Select Microsoft Edge TTS or Google Cloud TTS for Player announcements.'),
            '/pvoice <voice_name>': self._('Admins only: Set the Player announcement voice for the currently selected Player TTS mode.'),
            '/pvoices [langcode]': self._('List voices available for the current Player announcement TTS mode, optionally filtered by language code.'),
            '/pttsrate <-100..100>': self._('Admins only: Set Microsoft Player announcement speaking rate.'),
            '/pttsspeed <0.25..4.0>': self._('Admins only: Set Google Cloud Player announcement speaking speed.'),
        }
