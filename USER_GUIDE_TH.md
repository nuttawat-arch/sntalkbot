...
คำสั่งย่อถูกจัดให้หนึ่งคำสั่งหลักมี shorthand เดียว: Common ใช้ `a = about`, `sd = shutdown`, `sc = save`, `vt = voicetx`; Player/Full ใช้ `gl = l`, `c = select`, `sb = -`, `sf = +`; Manager/Full ใช้ `j = join` ส่วน alias ซ้ำหรือชนความหมายใหม่จะไม่ถูกย้อนกลับมา
...
`report <ข้อความ>` เป็น Common สำหรับส่งข้อความหาแอดมิน TeamTalk ที่ออนไลน์ จึงใช้ได้ทั้ง Player, Server Manager และ Full
...
- `dq <ลำดับ|ชื่อเพลง>` ลบเพียงรายการเดียวที่เลือก; `cq` ล้างรายการคิวแต่ไม่บังคับหยุดเสียงที่กำลังเล่น; `s` หยุด playback เท่านั้นโดยเก็บคิว/playlist/current item ไว้ และ `p` แบบไม่ใส่คำค้นจะเริ่มรายการเดิมใหม่จากต้น
...
ตั้งแต่ 5.1.8 ข้อมูล runtime แบบ realtime ใช้แหล่งเดียวคือ read-only HTTP API ของแต่ละ instance ซึ่ง bind เฉพาะ loopback และใช้ Bearer token; Web Manager รับข้อมูลจาก API แล้วส่งต่อ browser ผ่าน SSE โดยไม่มีไฟล์ snapshot JSON เป็น fallback อีกต่อไป หาก API ของ instance ใช้ไม่ได้ Web Manager จะแสดงว่าไม่มีข้อมูลสดแทนการนำข้อมูลเก่ามาแสดง ส่วนข้อมูลที่ต้องอยู่รอดข้าม restart เช่น Queue, timed moderation, notification, offline message และ preference จะเก็บใน SQLite/WAL ที่ persistent data ของ instance
