> **SNTalkBot 5.1.20:** All-in-One รุ่นปัจจุบัน — แยก Full/Player/Manager ชัดเจน, Queue/playlist/Related Radio, default YouTube cookies, dashboard `status`, recent `events` และ moderation ที่ตามทันการแก้ nickname/status/channel แบบ event-driven
...
ทั้ง Private และ Channel/Broadcast พิมพ์คำสั่งตรง ๆ ได้เหมือนกัน เช่น `h`, `p เพลง`, `ap on`, `rs` โดยไม่ต้องใส่ `/` ผู้ดูแลปิดการตอบสนองปกติต่อ Channel ด้วย `ci off` และเปิดด้วย `ci on`; `filter on|off` คุมการกรองคำทั้งหมด และ Manager/Full ใช้ `ic on|off` ควบคุมการดักข้อความจากทุก Channel
...
`validate_project.py` ต้องรายงานว่าไม่มี duplicate commands, help ครบทุก command, syntax เป็นแบบ prefix-free ทั้ง Private และ Channel/Broadcast (โดย `/` เป็นเพียง compatibility), ภาษาไทยไม่มีข้อความว่าง และไม่มีระบบ multi-profile หลงเหลือ
